...
from windows.BinaryParser import *
from windows.sid import *


TEST = b' \x00\x00\x00 \x02\x00\x00\x00\x00\x14\x00\x9d\x00\x02\x00\x01\x01\x00\x00\x00\x00\x00\x05\x0b\x00\x00\x00\x00\x00\x18\x00\x9d\x00\x02\x00\x01\x02\x00\x00\x00\x00\x00\x0f\x02\x00\x00\x00\x01\x00\x00\x00\x00\x008\x00\x9d\x00\x02\x00\x01\n\x00\x00\x00\x00\x00\x0f\x03\x00\x00\x00\x00\x04\x00\x00\xb01\x80?l\xbccL<\xe0P\xd1\x97\x0c\xa1b\x0f\x01\xcb\x19~z\xa6\xc0\xfa\xe6\x97\xf1\x19\xa3\x0c\xce\x01\x01\x00\x00\x00\x00\x00\x05\x12\x00\x00\x00\x01\x01\x00\x00\x00\x00\x00\x05\x12\x00\x00\x00'
...
class SID(Block, Nestable):
    ...
    def pretty_string(self):
        os = self.string()
        output=f"{os}"
        if get_well_known(os):
            output+=f"({get_well_known(os)})"
        return output
...
class SECURITY_DESCRIPTOR_RELATIVE(Block, Nestable):
    ...
    def pretty_string(self, indent=2, indent_level=0):
        output = ""
        owner = self.owner()
        group = self.group()

        if owner:
            output+=f"Owner: {owner.pretty_string()}\n"
        if group:
            output+=f"Group: {group.pretty_string()}\n"
        ...
        if dacl:
            ...
            if aces:
                ...
                indent_level+=1
                for ace in aces:
                    to_add = ace.pretty_string(indent, indent_level)
                    output+=f"{to_add}\n"
                indent_level -=1
        sacl = self.sacl()
        if sacl:
...
class SDS(Block):
    def __init__(self, buf, offset=0, parent=None):
...
def get_sid_string(data_bytes):
    sd = SECURITY_DESCRIPTOR_RELATIVE(data_bytes, 0, None)
    return sd.pretty_string(indent=1)
